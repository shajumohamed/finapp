...
import { CheckoutFormGradients } from '../gradients/checkout-form-gradients';
...
export function CheckoutContents({ userEmail }: Props) {
  ...
  useEffect(() => {
    if (!paddle?.Status && process.env.NEXT_PUBLIC_PADDLE_CLIENT_TOKEN && process.env.NEXT_PUBLIC_PADDLE_ENV) {
      initializePaddle({
  ...
  }, [paddle?.Status, priceId, userEmail]);

  useEffect(() => {
    if (paddle && priceId && paddle.Status) {
